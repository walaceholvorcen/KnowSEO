...
export default async function RelatorioPublico({
  ...
  const { eventos, artigos, contexto } = await carregarRelatorio(admin, dados.blogId, periodo);

  return (
    <div className="min-h-full bg-slate-50 dark:bg-background">
      <Relatorio blog={blog as Blog} periodo={periodo} eventos={eventos} artigos={artigos} contexto={contexto} publico />
    </div>
  );
}
